#!/usr/bin/env python3
"""
WHO-UMC Uppsala Monitoring Centre Proxy — auto-generated by MoltProxy.

Domain: who-umc.org
Tools: 7
Generated from MoltBook config.

Usage:
    echo '{"tool": "tool-name", "args": {}}' | python3 who_umc_org_proxy.py
"""

from __future__ import annotations

import json
import sys
import urllib.error
import urllib.parse
import urllib.request


import json

def ensure_str(val) -> str:
    """Coerce any input to string safely to prevent AttributeError."""
    if val is None:
        return ""
    if isinstance(val, (int, float, bool)):
        return str(val)
    if isinstance(val, (list, dict)):
        try:
            return json.dumps(val)
        except Exception:
            return str(val)
    return str(val)

def get_int_param(args: dict, key: str, default: int, min_val: int = None, max_val: int = None) -> int:
    """Safely parse integer parameter with optional clamping."""
    val = args.get(key)
    if val is None:
        return default
    try:
        res = int(val)
    except (ValueError, TypeError):
        return default
    if min_val is not None:
        res = max(res, min_val)
    if max_val is not None:
        res = min(res, max_val)
    return res




def _launch_browser():
    """Launch Playwright headless browser."""
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        return None, None, None
    pw = sync_playwright().start()
    browser = pw.chromium.launch(headless=True)
    page = browser.new_page()
    return pw, browser, page

BASE_URL = "https://who-umc.org"
DATA_SOURCE = "who-umc.org"
USER_AGENT = "NexVigilant-Station/1.0 (PV research tool)"
TIMEOUT = 15


def get_signal_methodology(args: dict) -> dict:
    """Get WHO-UMC signal detection methodology and vigiRank description"""

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def search_vigibase(args: dict) -> dict:
    """Search VigiBase for global ICSR data"""
    substance = ensure_str(args.get("substance", "")).strip()
    if not substance:
        return {"status": "error", "message": "substance is required"}
    reaction = args.get("reaction", "")

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def get_causality_assessment(args: dict) -> dict:
    """Get WHO-UMC causality assessment criteria and categories"""

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def get_country_programs(args: dict) -> dict:
    """Get list of national PV centres and their WHO Programme membership"""
    country = args.get("country", "")

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def get_naranjo_algorithm(args: dict) -> dict:
    """Get the Naranjo ADR Probability Scale — 10 questions with scoring criteria for causality assessment"""

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def get_ic_computation(args: dict) -> dict:
    """Get Information Component (IC) computation methodology with worked example — Bayesian signal detection metric used in VigiBase"""

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


def get_adverse_reaction_terminology(args: dict) -> dict:
    """Get WHO Adverse Reaction Terminology (WHO-ART) hierarchy description — legacy coding system with 4 levels"""

    pw, browser, page = _launch_browser()
    if pw is None:
        return {"status": "error", "message": "playwright not installed"}

    try:
        page.goto(BASE_URL, timeout=30000, wait_until="domcontentloaded")
        page.wait_for_timeout(3000)

        # TODO: Add CSS selectors for this tool
        # page.fill("selector", param_value)
        # page.click("selector")
        # results = page.query_selector_all("selector")

        page_text = page.inner_text("body")
        return {
            "status": "ok",
            "data": {"page_text_length": len(page_text)},
            "data_source": DATA_SOURCE,
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
    finally:
        browser.close()
        pw.stop()


TOOL_DISPATCH = {
    "get-signal-methodology": get_signal_methodology,
    "search-vigibase": search_vigibase,
    "get-causality-assessment": get_causality_assessment,
    "get-country-programs": get_country_programs,
    "get-naranjo-algorithm": get_naranjo_algorithm,
    "get-ic-computation": get_ic_computation,
    "get-adverse-reaction-terminology": get_adverse_reaction_terminology,
}


def main() -> None:
    raw = sys.stdin.read().strip()
    if not raw:
        print(json.dumps({"status": "error", "message": "No input on stdin"}))
        sys.exit(1)

    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as exc:
        print(json.dumps({"status": "error", "message": f"Invalid JSON: {exc}"}))
        sys.exit(1)

    tool_name = ensure_str(payload.get("tool", "")).strip()
    args = payload.get("arguments", payload.get("args", {}))

    if tool_name not in TOOL_DISPATCH:
        print(json.dumps({
            "status": "error",
            "message": f"Unknown tool '{tool_name}'. Known: {list(TOOL_DISPATCH.keys())}",
        }))
        sys.exit(1)

    result = TOOL_DISPATCH[tool_name](args)
    print(json.dumps(result, indent=2))


if __name__ == "__main__":
    main()
