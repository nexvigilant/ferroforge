...
fn test_registry_tool_count() {
    ...
    // 3 config tools + 7 meta tools (chart_course, directory, capabilities, station_health, ring_health, forge_diagnose, hop)
    assert_eq!(reg.tool_count(), 10);
}
...
fn test_route_directory_meta_tool() {
    ...
    assert_eq!(parsed["total_tools"], 10);
}
